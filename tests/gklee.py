...
if args.filename.suffix == ".yaml":
  with open(args.filename, 'r') as file:
    ...
    for file in yaml.safe_load(file):
      ...
      cont = client.containers.run(image="gklee", volumes=[
        f"{os.getcwd()}/tests:/mnt/tests"
      ], command="bash", detach=True, tty=True)

      def exec():
        base_name =  os.path.basename(os.path.splitext(filename)[0])
        (ec,out) = cont.exec_run(f"bash -c 'gklee-nvcc /mnt/{filename} -o {base_name}.bc -I/mnt/{os.path.dirname(filename)} -I/usr/include '")
        (ec,out) = cont.exec_run(f"bash -c 'timeout 1m gklee --verbose 0 --symbolic-config --race-prune {base_name}.bc'")
      ...
      if (profile):
        for i in range(iters):
          ...
          if str(filename) not in timings:
            timings[str(filename)] = { 'times': [dur], 'log': out}
          else: 
